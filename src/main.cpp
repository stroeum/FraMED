...
int main()
{
    ...
    {
        N.init(56,56,91);					// Number of discretization points
        L.init(22e+3,22e+3,31e+3);			// _m	Size of the simulation domain
        ...
        Q =    25;	Xq = L.x/2;	Yq = L.y/2-y_shift/2;	Zq = 4.50e+3+z_shift; Rq1 = 2.50e+3; Rq2=Rq1;	Rq3 = 0.50e+3;
        C.disk(Q, Xq,Yq,Zq, Rq1,Rq3, d,N);
        ChargeCfg.push_back(C);
        Q =	   -150;	Xq = L.x/2;	Yq = L.y/2-y_shift/2;	Zq = 7.5e+3+z_shift; Rq1 = 4.50e+3; Rq2=Rq1;	Rq3 = 1.250e+3;
        C.disk(Q, Xq,Yq,Zq, Rq1,Rq3, d,N);
        ChargeCfg.push_back(C);
        Q =    70;	Xq = L.x/2;	Yq = L.y/2+y_shift/2;	Zq = 13.0e+3+z_shift; Rq1 = 2.00e+3; Rq2=Rq1;	Rq3 = 1.25e+3;
        C.disk(Q, Xq,Yq,Zq, Rq1,Rq3, d,N);
        ChargeCfg.push_back(C);
        Q =    -1;	Xq = L.x/2;	Yq = L.y/2+y_shift/2;	Zq = 14.70e+3+z_shift; Rq1 = 2.00e+3; Rq2=Rq1;	Rq3 = 0.50e+3;
        C.disk(Q, Xq,Yq,Zq, Rq1,Rq3, d,N);
        ChargeCfg.push_back(C);
        ...
        // = 2 at point of maximum E-field magnitude
        // = 3 at implemented position
        rho3dCalculationStep	= 75;		// Charge density is calculated and store every rho3dCalculationStep steps.
    ...
    {
        N.init(101,101,201);				// Number of discretization points
        L.init(8e+3,8e+3,16e+3);			// _m	Size of the simulation domain
        d.init(L,N);						// _m	Sizes of grid-steps
        ...
        Ec.init(1*2.16e+5,1*2.16e+5,1*-2.16e+5, z_gnd,d,N,1);
        // _V/m Initiation-, Positive channel propagation-, Negative channel propagation-threshold
        ...
        InitZ	= 8e3;					// _m Z-coordinate of initiation point
        ...
        // Charge center parameters //
         ...
         //Q = 1; Xq = L.x/2;	Yq = L.y/2;	Zq = L.z/2;	Rq1 = 1.5e3;
...
//        ApplyBC(BCtype,phi,C.rho,d,N);
        ...
        bool uniformE = true;
        const double Eo = 1e5; // _V/_m
        double Vo = -Eo*(N.z-1)*d.z;
        double Xp = L.x/2;
        double Yp = L.y/2;
        double Zp = L.z;
        double Rp1 = L.x;
        double Rp2 = L.y;
        double Rp3 = d.z;
        // EndOf TEST WITH EQUIPOTENTIAL SURFACE //
        
        Potential V(Vo, Xp, Yp, Zp, Rp1, Rp2, Rp3, d, N);
        
//        for(int ii=0 ; ii<N.x ; ii++) for(int jj=0 ; jj<N.y ; jj++)	for(int kk=0 ; kk<N.z ; kk++)
//        {phi[ii][jj][kk]=-Eo*kk*d.z;};
        
        AddUniformE(uniformE, Eo, phi, Un, L, d, N);
                  ...
                  const StepsSizes dd, const BoxSteps NN)
{
    ...
    else if(IInitiationType == 4)
    {
        ...
        for(int ii=1; ii<NN.x-1; ii++) for(int jj=1; jj<NN.y-1; jj++) for(int kk=1; kk<NN.z-1; kk++)
            if(Eijk(ii,jj,kk, pphi,dd,NN)[0] >= EEc.initiation[kk])
                if(Eijk(ii,jj,kk, pphi,dd,NN)[0]/EEc.initiation[kk] >= Emax)
                {
                    Emax=Eijk(ii,jj,kk, pphi,dd,NN)[0]/EEc.initiation[kk];
                    IInitiationPoint.init(ii,jj,kk);
                };
        UUn[IInitiationPoint.i][IInitiationPoint.j][IInitiationPoint.k] = 1;
        pphi0 = pphi[IInitiationPoint.i][IInitiationPoint.j][IInitiationPoint.k];
