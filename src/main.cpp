...
void GrowTree(bool AAddNew, const bool iisBndXingPossible, const bool iisFlashAccoutedInBC,
			  ...
			  CMatrix3D& pphi, CMatrix3D& UUn, 
			  const BoxLengths& LL, const StepsSizes dd, const BoxSteps NN);
...
int main()
{
	...
	for(int ii = 0; ii <1e3 ; ii++)	random()/(double)RAND_MAX;							//Force random seed
	...
	double			InitR;					// Radius of the initiation region (if applicable)
	...
	int				BCtype;					// Boundary conditions:
											// = 0 ``Tin Can"
											// = 1 Open BC
	...
	int				rho3dCalculationStep;	// Charge density is calculated and store every rho3dCalculationStep steps. (0: 3-D charge density is never calculated)

	bool			isChargeContentAssumed;	// Choose either charge content or loading currents
	bool			isInitiationPrevented;	// Only simulate cloud electrical structure
	bool			AddNew;					// Channel is allowed to propagate: Y/N
	bool			isBCerrorCalculated;	// Error at the boundary is calculated at each step: Y/N
	bool			isBndXingPossible;		// Channel is allowed to cross boundaries: Y/N
	bool			isChannelEquipotential;	// Channel potential is adapted to ensure charge neutrality: Y/N
	bool			isEsEnergyCalculated;	// Electrostatic energy is calculated: Y/N
	bool			isFlashAccoutedInBC;	// Channel charge is accounted for in derivation of BC: Y/N
	bool			isInitiationPossible;	// Initiation is possible in the simulation domain after charge load: Y/N
	bool			isLinkXingPossible;		// Channels crosses are allowed: Y/N
	bool			isRsDeveloped;			// Return stroke development: Y/N
	...
	isChargeContentAssumed	= true;			// Choose either charge content or loading currents

	if(isChargeContentAssumed == false)
	{
		N.init(41,41,61);					// Number of discretization points
		L.init(12e+3,12e+3,12e+3);			// _m	Size of the simulation domain
		...
		z_gnd   = 3e3;						// _m Altitude of ground level
		...
		Ec.init(216e+3,216e+3,-216e+3, z_gnd,d,N);
											// _V/m Initiation-, Positive channel propagation-, Negative channel propagation-threshold
		Vd.init(0e+5,-0e+5, z_gnd,d,N);
											// _V/m Voltage drop in positive, negative channels
		// CG //
		InitX	= 27e3;						// _m X-coordinate of initiation point
		InitY	= 20e3;						// _m Y-coordinate of initiation point
		InitZ	= 5.55e3;					// _m Z-coordinate of initiation point
		InitR	= 0;						// _m Radius of the initiation region (if applicable)
		...
		I1		= 1.5;						// _A Loading current I1
		I2		= -90e-3;					// _A Loading current I2	
		Iscreen	= 0*664e-3;					// _A Screening current Iscreen
		
		// Charge center parameters //
		// Q (C) Charge content; Xq,Yq,Zq (m) Charge center coordinate; R,H (m) Size of the charge center //
		Q =    0;	Xq = L.x/2;	Yq = L.y/2-y_shift/2;	Zq = 2.00e+3+z_shift; Rq1 = 1.50e+3;	Rq3 = 1.50e+3;
		C.disk(Q, Xq,Yq,Zq, Rq1,Rq3, d,N);
		ChargeCfg.push_back(C);
		Q =	   0;	Xq = L.x/2;	Yq = L.y/2-y_shift/2;	Zq = 3.75e+3+z_shift; Rq1 = 3.00e+3;	Rq3 = 1.50e+3;  
		C.disk(Q, Xq,Yq,Zq, Rq1,Rq3, d,N);
		ChargeCfg.push_back(C);
		Q =    0;	Xq = L.x/2;	Yq = L.y/2+y_shift/2;	Zq = 6.75e+3+z_shift; Rq1 = 4.00e+3;	Rq3 = 1.50e+3;  
		C.disk(Q, Xq,Yq,Zq, Rq1,Rq3, d,N);
		ChargeCfg.push_back(C);
		Q =    0;	Xq = L.x/2;	Yq = L.y/2+y_shift/2;	Zq = 8.00e+3+z_shift; Rq1 = 4.00e+3;	Rq3 = 0.50e+3;
		C.disk(Q, Xq,Yq,Zq, Rq1,Rq3, d,N);
		ChargeCfg.push_back(C);
		// Endof Charge center parameters //
				
		ThresholdOvershoot		= 10;		// % by which the E-field initiation threshold must be exceeded 
											// in the simulation domain in order to start a flash(for initiation threshold)
		BCtype					= 1;		// Boundary conditions:
											// = 0 ``Tin Can"
											// = 1 Open BC
											// = 2 Moving Capacitor Plates
											// = 3 Free Space
		InitiationType			= 1;		// Initiation type:
											// = 1 randomly in available region
											// = 2 at point of maximum E-field magnitude
											// = 3 at implemented position
		rho3dCalculationStep	= 50;		// Charge density is calculated and store every rho3dCalculationStep steps.
		...
		isFlashAccoutedInBC 	= true;		// Channel charge is accounted for in derivation of BC: Y/N
		isInitiationPossible	= false;	// Initiation is possible in the simulation domain after charge load: Y/N (automatically turned to true if initition is possible)
		isLinkXingPossible		= false;	// Channels crosses are allowed: Y/N
		isRsDeveloped			= true;		// Return stroke development: Y/N

		cout<<"/*******************************************************************/"<<endl;
		cout<<"/* Load charge layers                                              */"<<endl;
		cout<<"/*******************************************************************/"<<endl;
		
		ThresholdOvershoot		/= 100;		// Convert % into decimal

		LoadTripoleModel(ChargeCfg, I1,I2,Iscreen,
						 SOR,epsilon,MaxStep,BCtype, Ec, ThresholdOvershoot,
						 phi,Un, d,N);
	...
	else if(isChargeContentAssumed == true)
	{
		N.init(41,41,61);					// Number of discretization points
		L.init(12e+3,12e+3,12e+3);			// _m	Size of the simulation domain
		d.init(L,N);						// _m	Sizes of grid-steps
		...
		//	We assume that the first link is somehow established, then only the propagation threshold needs to be exceeded to develop the flash.
		Ec.init(2.16e+5,2.16e+5,-2.16e+5, z_gnd,d,N);
											// _V/m Initiation-, Positive channel propagation-, Negative channel propagation-threshold
		Vd.init(0e+5,-0e+5, z_gnd,d,N);
											// _V/m Voltage drop in positive, negative channels
		...
		InitZ	= 7.6e3;					// _m Z-coordinate of initiation point
		...
		// Charge center parameters //
		/*
		Q = +80;		Xq = L.x/2;	Yq = L.y/2;	Zq = 6e+3;	Rq1 = 1.5e3;
		C.sphere(Q, Xq,Yq,Zq, Rq1, d,N);
		CMatrix1D TEST(C.MonopoleAnalyticalSolution(d,N));
		TEST.fwrite("results/test.dat");
		ChargeCfg.push_back(C);
		*/
		/*
		Q = 19;	Xq = L.x/2;	Yq = 6.0e3+y_shift;	Zq = 2.00e+3+z_shift;	Rq1 = .90e+3; Rq2=Rq1;		Rq3 = 0.85e+3;
		C.gaussian(Q, Xq,Yq,Zq, Rq1,Rq2,Rq3, d,N);
		ChargeCfg.push_back(C);
		Q =-90;	Xq = L.x/2;	Yq = 6.0e3+y_shift;	Zq = 3.75e+3+z_shift;	Rq1 = 1.75e+3; Rq2=Rq1;		Rq3 = 1.25e+3;  
		C.gaussian(Q, Xq,Yq,Zq, Rq1,Rq2,Rq3, d,N);
		ChargeCfg.push_back(C);
		Q = 65;	Xq = L.x/2;	Yq = 6.0e3+y_shift;	Zq = 6.75e+3+z_shift;	Rq1 = 2.25e+3; Rq2=Rq1;		Rq3 = 1.25e+3;  
		C.gaussian(Q, Xq,Yq,Zq, Rq1,Rq2,Rq3, d,N);
		ChargeCfg.push_back(C);
		Q =-30;	Xq = L.x/2;	Yq = 6.0e3+y_shift;	Zq = 8.00e+3+z_shift;	Rq1 = 2.00e+3; Rq2=Rq1;		Rq3 = 0.50e+3;  
		C.gaussian(Q, Xq,Yq,Zq, Rq1,Rq2,Rq3, d,N);
		ChargeCfg.push_back(C);
		*/
		Q = -5.0+0*.65* 10.0;	Xq = L.x/2;	Yq = 6.0e3+y_shift;	Zq = 2.00e+3+z_shift;	Rq1 = .90e+3; Rq2=Rq1;		Rq3 = 0.85e+3;
		C.gaussian(Q, Xq,Yq,Zq, Rq1,Rq2,Rq3, d,N);
		ChargeCfg.push_back(C);
		Q = 40.0+0*.65*-65.0;	Xq = L.x/2;	Yq = 6.0e3+y_shift;	Zq = 3.75e+3+z_shift;	Rq1 = 1.75e+3; Rq2=Rq1;		Rq3 = 1.25e+3;  
		C.gaussian(Q, Xq,Yq,Zq, Rq1,Rq2,Rq3, d,N);
		ChargeCfg.push_back(C);
		Q =-57.5+0*.65* 95.0;	Xq = L.x/2;	Yq = 6.0e3+y_shift;	Zq = 6.75e+3+z_shift;	Rq1 = 2.00e+3; Rq2=Rq1;		Rq3 = 1.00e+3;  
		C.gaussian(Q, Xq,Yq,Zq, Rq1,Rq2,Rq3, d,N);
		ChargeCfg.push_back(C);
		Q = 20.0+0*.65*-35.0;	Xq = L.x/2;	Yq = 6.0e3+y_shift;	Zq = 8.00e+3+z_shift;	Rq1 = 1.25e+3; Rq2=Rq1;		Rq3 = .35e+3;  
		C.gaussian(Q, Xq,Yq,Zq, Rq1,Rq2,Rq3, d,N);
		ChargeCfg.push_back(C);
		// Endof Charge center parameters //
		...
		BCtype					= 1;		// Boundary conditions:
											...
											// = 2 Moving Capacitor Plates
											// = 3 Free Space
		InitiationType			= 3;		// Initiation type:
											// = 1 randomly in available region
											// = 2 at point of maximum E-field magnitude
											// = 3 at implemented position
	...
	cout<<"/*******************************************************************/"<<endl;
	cout<<"/* Find Potential Extrema                                          */"<<endl;
	cout<<"/*******************************************************************/"<<endl;
	...
	cout<<"/*******************************************************************/"<<endl;
	cout<<"/* Estimate field & Store                                          */"<<endl;
	cout<<"/*******************************************************************/"<<endl;
				  ...
				  const StepsSizes dd, const BoxSteps NN)
{
	...
	cout<<"Initx = "<<IInitiationPoint.i*dd.x<<endl;
	cout<<"Inity = "<<IInitiationPoint.j*dd.y<<endl;
	cout<<"Initz = "<<IInitiationPoint.k*dd.z<<endl;	
//	SSOR.Solve(dd,NN,UUn,pphi);
